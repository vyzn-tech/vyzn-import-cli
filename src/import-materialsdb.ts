...
export async function importMaterialsDb(url: string, auth: string, category: string, verbose: boolean) {
    ...
    for(const company of producerIndex.MaterialsDBIndex.company) {
        ...
        for(const material of producerData.materials.material) {
            ...
            for(const layer of material.layers[0].layer) {
                        ...
                        if(lcaEntry && lcaEntry.$ && lcaEntry.$.database == lcaDatabase && lcaEntry.lcaversion && lcaEntry.lcaversion.length) {
                            for(const lcaVersion of lcaEntry.lcaversion) {
                                if(lcaVersion && lcaVersion.$ && lcaVersionNames.includes(lcaVersion.$.id)) { 
                                    ...
                                    if(lcaVersionNames[0] == lcaVersion.$.id) break // stop searching if its the newest version, also assumes that the records are in-order by version
                                }
                            }
                        }
                    }
                ...
                if(thick) pushAttr("vyzn.catalogue.Thickness", parseFloat(thick))
            ...
            /*
            let name = material.information[0].names[0].name[0]
            if(typeof name !== "string") {
                name = null
                for(const translatedName of material.information[0].names[0].name) {
                    if(translatedName.$.lang === "de") {
                        name = translatedName._
                        break
                    }
                }
                if(!name)
                    name = material.information[0].names[0].name[0]._
            }*/ 
        }

        //fs.writeFile("producerData.json", JSON.stringify(producerData))
        //console.log(JSON.stringify(producerData))
    }

    //if(verbose)
        //console.debug(JSON.stringify(jObj))
}

function getTranslatedString(obj: any, attribute: string, lang: string) {
