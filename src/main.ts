...
=======
  ...
  program
  .command('import-catalog-components')
  .description('import catalog from a JSON file')
  .requiredOption('-i, --input <file>', 'path to the file to import (.json)')
  .requiredOption('-u, --url <url>', 'The URL of the vyzn API')
  .requiredOption('-a, --auth <file>', 'The file containing the auth token')
  ...
  .option('-d, --diff', 'Perform diff only')
  .action((o) => {
    importCatalogComponents(o.input, o.url, o.auth, o.category, o.verbose, o.diff)
  })
...
async function importCatalogComponents(input: string, url: string, auth: string, category: string, verbose: boolean, diff: boolean) {
  const lcaAttributeGroup = 'Ökobilanz'
  ...
  //await importProductsOfType(componentsObj.products, "MATERIAL", selectedCatalogueId, hierarchy,"", productTypeNameToCategoryTypeIdMap, lcaAttributeGroupId, url, authToken, verbose, diff)
  await importProductsOfType(componentsObj.products, "COMPONENT", selectedCatalogueId, selectedRoot, hierarchy, productTypeNameToCategoryTypeIdMap, lcaAttributeGroupId,url, authToken, verbose, diff)
}

let anchorFound = false

async function importProductsOfType(products, type: string, selectedCatalogueId, selectedRoot, hierarchy, productTypeNameToCategoryTypeIdMap, lcaAttributeGroupId: string, url: string, auth: string, verbose: boolean, diff: boolean) {
  for (const [key, value] of Object.entries(products)) {
    let prod: any = value
    if (prod.type != type) continue

    //if(!prod.categoryPath.endsWith('Swiss Property AG')) continue
    ...
    await importSingleProduct(key, prod, selectedCatalogueId, selectedRoot, hierarchy, productTypeNameToCategoryTypeIdMap, lcaAttributeGroupId, url, auth, verbose, diff)
  }
}

async function importSingleProduct(prodKey, prod, selectedCatalogueId, selectedRoot, hierarchy, productTypeNameToCategoryTypeIdMap, lcaAttributeGroupId: string, url: string, authToken: string, verbose: boolean, diff: boolean) {
