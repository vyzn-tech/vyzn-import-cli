...
async function importCatalog(input, url, auth, tenant, verbose, diff, folder, category, importRefMat, importMat, importBuildTech, importOtRes, importComp, overriding) {
    ...
    const attributeGroupsByPosition = {};
    ...
    if (importRefMat)
        await importProductsOfType(componentsObj.products, "REFERENCE_MATERIAL", selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding);
    if (importMat)
        await importProductsOfType(componentsObj.products, "MATERIAL", selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding);
    if (importBuildTech)
        await importProductsOfType(componentsObj.products, "BUILDING_TECHNOLOGY", selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding);
    if (importOtRes)
        await importProductsOfType(componentsObj.products, "OTHER_RESOURCE", selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding);
    if (importComp)
        await importProductsOfType(componentsObj.products, "COMPONENT", selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding);
}
let anchorFound = false;
async function importProductsOfType(products, type, selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, auth, tenant, verbose, diff, folder, category, overriding) {
...
async function importSingleProduct(prodKey, prod, selectedCatalogueId, hierarchy, lcaAttributeGroupId, attributeGroupsByPosition, url, authToken, tenant, verbose, diff, folder, category, overriding) {
    ...
    if (prod.type == "MATERIAL") {
        ...
        const linkedRefs = Array.isArray(rawLinked)
            ? rawLinked
